                ...
                return Column(
                  children: [
                    Padding(
                      ...
                      child: Text("No se encontró el producto que buscabas",
                      style: TextStyle(fontSize: 20),),
                    )
                  ],
                );
                }
              return BuildIsoPosts(isoPostsStream: _isoPostsStream, multicontroller2: _multicontroller2, currentUserId: currentUserId);
  }
          ),
    )
    );
  ...
  return Column(
    ...
    children: [
      ...
      Padding(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween, // Pushes elements to opposite edges
...
class BuildIsoPosts extends StatelessWidget {
  const BuildIsoPosts({
    super.key,
    required Stream<QuerySnapshot<Object?>> isoPostsStream,
    required MultiSelectController<String> multicontroller2,
    required this.currentUserId,
  }) : _isoPostsStream = isoPostsStream, _multicontroller2 = multicontroller2;

  final Stream<QuerySnapshot<Object?>> _isoPostsStream;
  final MultiSelectController<String> _multicontroller2;
  final String currentUserId;

  @override
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
    stream: _isoPostsStream,
                ...
                BlocBuilder<StatusFilterHistorySelectionCubit, List<String>>(
                      builder: (context, state) {
                        if (context.read<StatusFilterHistorySelectionCubit>().selectedFilters.contains("Todos")){
                          return listTransactions(context, listEntities, ScrollController(), currentUserId);
                        }
